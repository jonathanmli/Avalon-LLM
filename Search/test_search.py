...
if __name__ == "__main__":
    ...
    action_enumerator = GPT35ActionEnumerator(model)
    value_heuristic = GPT35ValueHeuristic(model)
    opponent_action_predictor = GPT35OpponentActionPredictor(model)
    opponent_action_enumerator = GPT35OpponentActionEnumerator(model)
    hidden_state_predictor = GPT35RandomStatePredictor(model)
    hidden_state_enumerator = GPT35RandomStateEnumerator(model)
    forward_predictor = GPT35ForwardPredictor(model)
    forward_enumerator = GPT35ForwardEnumerator(model)
