...
GENERATION = "Given the discussion history, current state, and your intended action, produce dialogue that would help you achieve your intent. Dialogue aimed at persuasion should be presented in a convincing manner, supported by ethos, logos, and pathos. Note that dialogue will be seen by all players in the game."
...
class PromptGenerator:
    ...
    def generate_pgood_belief_discrimination_prompt(self, history: str, players: set, private_information: str):
        discrim_str = f"Based on the discussions this round, for each player in players {players}, do you think the probability of the player being Evil (2) increased significantly (1) increased slightly (0) stayed the same (-1) decreased slightly or (-2) decreased significantly and why? End with a dictionary of player to your multiple choice answer, i.e. {{0: (2, 'increased significantly'), ..., 4: (0, 'stayed the same')}}"
        return self.sys_prompt + self.rules + private_information + history + discrim_str
    
    def generate_pmerlin_belief_discrimination_prompt(self, history: str, players: set, private_information: str):
        discrim_str = f"Based on the discussions this round, for each player in players {players}, do you think the probability of the player being Merlin (2) increased significantly (1) increased slightly (0) stayed the same (-1) decreased slightly or (-2) decreased significantly and why? End with a dictionary of player to your multiple choice answer, i.e. {{0: (2, 'increased significantly'), ..., 4: (0, 'stayed the same')}}"
        return self.sys_prompt + self.rules + private_information + history + discrim_str
    ...
    def gen_dialogue_description(dialogue: list[str], speakers: list[int]) -> str:
        ...
        dialogue_str = "This discussion round players have made the following comments so far:"
