...
class ForwardTransitor(ABC):
    ...
    '''
    def transition(self, state: Hashable, action: Hashable)->Tuple[Hashable, dict[int, float]]:
        '''
        ...
        '''
        state, rewards = self._transition(state, action)
    ...
    @abstractmethod
    def _transition(self, state: Hashable, action: Hashable)->Tuple[Hashable, dict[int, float]]:
        '''
        Transits to the next state given the current state and action
