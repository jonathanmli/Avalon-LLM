...
class NaiveSpeakableAgent(Agent):
...
class NaiveSpeakableMinion(NaiveSpeakableAgent):
    
    def __init__(self, id: int, name: str, config: AvalonBasicConfig, side: int=0, role_name: str="Minion", session: AvalonSessionWrapper=None, role: int=6, sides: List[int]=None, **configs):
        assert role == 6
        super().__init__(
            id      =   id,
            name    =   name,
...
class NaiveSpeakableAssassin(NaiveSpeakableAgent):
    
    def __init__(self, id: int, name: str, config: AvalonBasicConfig, side: int=0, role_name: str="Assassin", session: AvalonSessionWrapper=None, role: int=7, sides: List[int]=None, **configs):
        assert role == 7
        super().__init__(
            id      =   id,
            name    =   name,
...
class NaiveSpeakableMerlin(NaiveSpeakableAgent):

    def __init__(self, id: int, name: str, config: AvalonBasicConfig, session: AvalonSessionWrapper=None, side: int=1, role: int=0, sides: List[int]=None, **configs):
        assert role == 0
        super().__init__(
            id      =   id,
            name    =   name,
...
class NaiveSpeakableServant(NaiveSpeakableAgent):

    def __init__(self, id: int, name: str, config: AvalonBasicConfig, side: int=1, role: int=5, session: AvalonSessionWrapper=None, sides: List[int]=None, lexigraphic: bool=True, **configs):
        assert role == 5
        super().__init__(
            id      =   id,
            name    =   name,
...
SPEAK_NAIVEAGENT_FINDER = {
    'Servant': NaiveSpeakableServant,
    'Merlin': NaiveSpeakableMerlin,
    'Assassin': NaiveSpeakableAssassin,
    'Minion': NaiveSpeakableMinion
}

def find_naive_speakable_agent(**configs):
    return SPEAK_NAIVEAGENT_FINDER[configs["role_name"]](
        name        =   configs["name"],
...
if __name__ == "__main__":
    ...
    config = env.config
    player_0 = find_naive_speakable_agent['Merlin'](0, 'player_0', config)
